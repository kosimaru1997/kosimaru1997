...
重要なバッチの処理に問題が多く実行に時間がかかっていたため、コードの見直しを行い、バッチ処理の改善を1/4程度まで速度の短縮をしました。

また、CloudWatchを用いたログ監視体制が存在していなかったため、TraceBackが発生した場合は即Slackに通知させる用に設定し、障害発生時の対応を迅速に行える体制を整えるとともに、RDSのスローログなどプロジェクトのキーとなるログの確認方法を共有し障害発生時の対応を迅速に行える体制を整えました。
